...
`xaos.Towns.main()` first shows the pre-launch launcher (`xaos.launcher.Launcher`: a small branded GLFW window for the pre-launch settings; window size, fullscreen, audio, enabled mods; Play saves them to the user-folder towns.ini overlay and continues, closing it exits; skippable with `-Dtowns.skipLauncher=true`). The proprietary-asset setup is part of the launcher: when the assets are missing, Play is disabled and an assets row offers to copy them from a detected Steam install or a browsed folder (reusing `FirstRunSetup`/`SteamLocator`, copy on a background thread). Then main runs the first-run asset setup (`xaos.setup.FirstRunSetup`: if `data/graphics`, `data/audio` or `data/fonts` are missing from the working directory, it locates a Steam Towns install or asks for one via native tinyfd dialogs and copies them in), then initializes the Steam API via JNA if present (failure is silently ignored; the game runs fine without Steam), then constructs `xaos.main.Game`. Game creates the user folder, reads config, opens a GLFW window through the compatibility layer (`xaos.compat.opengl.Display`), initializes OpenAL audio (`utils.UtilsAL`), and enters the main loop in `Game.run()`: poll input, advance the simulation one turn, render, swap buffers, cap the frame rate (`FPS_MAINMENU` and `FPS_INGAME` in towns.ini, both 30 by default).
...
- `xaos.launcher`: the pre-launch settings window, shown from `Towns.main()` before anything else. Raw GLFW + immediate-mode GL with system-font text baked via `stb_truetype` (the game's font assets may not exist yet at this point; `stb_easy_font` is the no-system-font fallback) and the repo logo from `packaging/icons/`; fully torn down (including `glfwTerminate`) before the game's own window layer starts. Reads both towns.ini layers itself and writes only the user-folder overlay, preserving keys it does not manage. Hosts the asset setup UI (Play gated on assets present, Steam-detect + browse buttons) on top of the `xaos.setup` logic. Imports only `xaos.setup` and nothing from the game packages: it must not class-initialize `Game` or draw RNG.
- `xaos.setup`: asset detection/validation/copy logic, used by the launcher; its own `run()` dialog flow still runs from `Towns.main()` after the launcher as a no-op fallback (covers `-Dtowns.skipLauncher=true`). `SteamLocator` finds a Steam Towns install (registry/default paths + a dumb `libraryfolders.vdf` line scan); `FirstRunSetup` validates, confirms via tinyfd native dialogs (message box + folder picker; no AWT, which would fight GLFW on macOS) and copies `data/graphics`, `data/audio`, `data/fonts` into the working directory (tmp-then-rename per folder, so an interrupted copy never looks complete). Imports nothing from the game packages: it must not class-initialize `Game` or draw RNG. `TownsHeadless` never calls it.
...
**Scripted-directive scenarios** extend the pinned surface from passive simulation to *active play*: a scenario issues player orders and pins what the citizens do in response. The harness lives alongside the other tests:

- `Orders` issues one order through the same model seam the UI drives (`Game.createTask` -> `Task.setPoint` -> `Game.taskCreated`), without any presentation. It never sets a cursor `Tile`, so issuing an order draws no `Utils.random` number and does not perturb the RNG stream.
- `Directive` is one scheduled order as plain data (task type, target cells, optional parameter); a scenario is just an ordered list of them. `ScenarioRunner` boots a seeded headless world, applies each directive at its tick (in list order, before that tick's `nextTurn`), advances with synchronous pathfinding, and records terrain/state hashes at checkpoints into a `ScenarioResult`.
- `ScenarioMineTest` is the first scenario: it issues one mine order next to the spawn and asserts both that the order was *accepted* (the cell is flagged, a task registers) and *carried out* (the cell is mined within the tick budget), so a green pin cannot silently freeze a dropped or no-op order, then pins the resulting hashes. Its pins use the `scenario.*` keys in `Golden.java` and follow the same update workflow. Same determinism contract and one-game-per-JVM rule as the other in-JVM tests.
